...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to pipe where commands are dumped",
)
parser.add_argument(
    "--pipe-output-dir",
    type=str,
    required=True,
    help="Path to command result output directory",
)
...
while True:
    ...
    try:
        ...
        command_obj = {
            "cmd": "",
            "outputPath": os.path.join(args.pipe_output_dir, "output.txt"),
        }
    ...
    output_path = os.path.join(
        args.pipe_output_dir, os.path.basename(command_obj["outputPath"])
    )
